...
type CharacterMaterials = {
  levelingMaterials: Array<{
    levelRange: string;
    materials: Array<{ id: string; quantity: number }>;
  }>;
  skillMaterials: Array<{ id: string; quantity: number }>;
};
...
interface BuildCalculatorClientProps {
  lang: Locale;
  characters: BuildCharacter[];
  weapons: BuildWeapon[];
  builds: CharacterBuild[];
  materialsById: Record<string, BuildMaterial>;
  characterMaterialsById: Record<string, CharacterMaterials>;
}

export function BuildCalculatorClient({
  lang,
  characters,
  weapons,
  builds,
  ...
  const levelingMaterials = useMemo(() => {
    if (!selectedCharacter) return null;
    return calculateMaterialsForRange(characterMaterialsById[selectedCharacter], 1, 60);
