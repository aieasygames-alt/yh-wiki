  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-3 border-b border-gray-800/50 last:border-b-0 ${done ? "opacity-60" : ""}`}>
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        {done && (
          <svg className="w-4 h-4 text-emerald-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
...
export default function MaterialPlannerPage() {
  ...
  const addCharacter = (charId: string) => {
    if (plan.some((p) => p.characterId === charId)) return;
  ...
  const removeCharacter = (charId: string) => {
    const newPlan = plan.filter((p) => p.characterId !== charId);
    setPlan(newPlan);
    savePlan(newPlan, owned);
  };

  const updateEntry = (charId: string, updates: Partial<PlanEntry>) => {
    const newPlan = plan.map((p) => p.characterId === charId ? { ...p, ...updates } : p);
    setPlan(newPlan);
    savePlan(newPlan, owned);
  };
  ...
  const aggregated = useMemo(() => {
    ...
    for (const entry of plan) {
      const levelMats = calculateMaterials(entry.characterId, entry.currentLevel, entry.targetLevel);
      for (const m of levelMats) {
        totals[m.materialId] = (totals[m.materialId] || 0) + m.quantity;
      }
      if (entry.includeSkills) {
        const skills = getCharacterMaterials(entry.characterId)?.skillMaterials || [];
        for (const m of skills) {
          totals[m.id] = (totals[m.id] || 0) + m.quantity;
        }
      }
    }
    return Object.entries(totals)
      ...
      .sort((a, b) => {
        ...
        return (mb?.rarity || 0) - (ma?.rarity || 0);
      });
  }, [plan]);

  const totalMats = aggregated.length;
  const completedMats = aggregated.filter((m) => (owned[m.materialId] || 0) >= m.quantity).length;
  const overallPct = totalMats > 0 ? Math.round((completedMats / totalMats) * 100) : 0;

  // Filter characters not already in plan
  const availableChars = useMemo(() => {
    const plannedIds = new Set(plan.map((p) => p.characterId));
  ...
  }, [allCharacters, plan, searchQuery]);
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-4">
              ...
              <div className="grid grid-cols-4 gap-2 max-h-48 overflow-y-auto">
                {availableChars.slice(0, 20).map((c) => (
                  <button
                    key={c.id}
                    onClick={() => { addCharacter(c.id); setSearchQuery(""); }}
            ...
            {plan.length > 0 && (
              <div className="rounded-xl border border-gray-800 bg-gray-900/50 p-4 space-y-3">
                ...
                {plan.map((entry) => {
                  const char = allCharacters.find((c) => c.id === entry.characterId);
                  if (!char) return null;
                  return (
                    <div key={entry.characterId} className="rounded-lg border border-gray-800 bg-gray-800/30 p-3">
                      <div className="flex items-center gap-2 mb-2">
                        <GameImage type="character" id={char.id} name={char.name} className="w-8 h-8 rounded shrink-0" />
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium truncate">{isZh ? char.name : char.nameEn}</p>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <input
                            type="number" min={1} max={entry.targetLevel - 1}
                        ...
                        <div>
                          ...
                          <input
                            type="number" min={entry.currentLevel + 1} max={60}
                ...
                <div className="rounded-xl border border-gray-800 bg-gray-900/50 overflow-hidden">
                  {aggregated.map((m) => (
                    <MaterialRow
                      ...
                      matId={m.materialId}
                      totalQty={m.quantity}
                      owned={owned[m.materialId] || 0}
                      onOwnedChange={(v) => setOwnedQty(m.materialId, v)}
                      locale={lang}
                      lang={lang as string}
                    />
                  ))}
