...
export default async function VehiclesPage({
  ...
  const groupedVehicles = vehicles.reduce((acc, v) => {
    const type = locale === "zh" ? v.type : v.typeEn;
    ...
    <>
      ...
      <Breadcrumb
        items={[
          ...
          { label: locale === "zh" ? "载具" : "Vehicles" },
        ]}
      />
      <div className="max-w-6xl mx-auto px-4 py-12">
        <div className="text-center mb-12">
          <h1 className="text-3xl font-bold mb-4">
            {locale === "zh" ? "载具图鉴" : "Vehicle Database"}
          </h1>
        ...
        <div className="mt-12 p-6 rounded-xl border border-gray-800 bg-gray-900/50">
          <h2 className="text-lg font-bold mb-4">
            {locale === "zh" ? "载具统计" : "Vehicle Statistics"}
          </h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
            <div>
              <p className="text-2xl font-bold text-primary-400">{vehicles.length}</p>
              <p className="text-sm text-gray-400">{locale === "zh" ? "总载具数" : "Total Vehicles"}</p>
            </div>
            <div>
              <p className="text-2xl font-bold text-yellow-400">
                {vehicles.filter((v) => v.rarity >= 5).length}
              </p>
              <p className="text-sm text-gray-400">{locale === "zh" ? "5星载具" : "5-Star"}</p>
            </div>
            <div>
              <p className="text-2xl font-bold text-green-400">
                {vehicles.filter((v) => v.speed === "S").length}
              </p>
              <p className="text-sm text-gray-400">{locale === "zh" ? "S级速度" : "S-Rank Speed"}</p>
            </div>
            <div>
              <p className="text-2xl font-bold text-red-400">
                {vehicles.filter((v) => v.price !== "-").length}
              </p>
              <p className="text-sm text-gray-400">{locale === "zh" ? "商店可购买" : "Shop Available"}</p>
            </div>
          </div>
        </div>
      </div>
